...
<?php
 ...
 if ($_GET['acces'] == 'validation')
 {
  ...
  { 
   $req = "UPDATE blog SET visible = '$v_valeur' WHERE forum = '$forum' and numero = '$v_numero'"; 
   $status = query ($req);
  }
 }
 ...
 function afficher_message ($r, $mode)
 {
  ...
  echo format_value($r->titre);
  echo '"><input type=hidden name=r_motscles value="';
  echo format_value($r->motscles);
  echo '">';
  echo '</form>';
 ...
 function afficher_arbo ($n, $tri, $mode)
 {
   ...
   $req1 = "SELECT * FROM blog WHERE forum = '$forum' AND reponsea = $n";
...
Mots clés : <input type=text name=motscles size=60 value="<?php 
 if ($operation == 'repondre')
  echo format_value($r_motscles);
 else if ($operation == 'modifier')
  echo format_value($m_motscles);
?>">
<br>
Texte : 
<br>
...
<?php
 if ($operation == 'modifier')
 {
  ...
  $req="SELECT * FROM blog WHERE forum = '$forum' AND numero = $m_numero";
  /* echo $req; */
  $data=query($req);
  $r=fetch_object($data);
  echo $r->texte;
...
<h3>Options d'affichage</h3>
  ...
  else if ($mode == 'lin')
  {
   ...
   else
   {
    ...
    $req = "SELECT * FROM blog WHERE forum = '$forum'";
    if ($_GET['acces'] != 'validation')
...
<?php
  ...
  if ($enreg == 'oui')
  {
   ...
   if ($operation == 'modifier')
   {
    $req = "DELETE FROM blog WHERE forum = '$forum' AND numero = $numero_a_modifier AND motdepasse = '$motdepasse'";
    echo '<br>' . $req . '<br>';
    $status = query ($req);
   }

   {
    ...
    $moscles1 = treat_string($motscles);
    $auteur1 = treat_string($auteur);
    $texte1 = treat_string($texte);
	*/

	$titre1 = format_query(format_input($titre));
	$motscles1 = format_query(format_input($motscles));
	$auteur1 = format_query(format_input($auteur));
	$texte1 = format_query(format_input($texte));	

    $req = "INSERT INTO blog (forum, titre, reponsea, modifde, motscles, auteur, motdepasse, visible, texte, date) VALUES ('$forum', '$titre1', $reponsea, $numero_a_modifier, '$motscles1', '$auteur1', '$motdepasse1', $validauto, '$texte1', $auj)";
    /* echo $req; */
    $status = query ($req);
    if (!$status)
    {
