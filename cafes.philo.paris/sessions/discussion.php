...
<?php
 ...
 if (isset($_GET['supprimer']) && is_numeric($_GET['supprimer']) && (($rec->statut & 4) == 0 || $_SESSION['moderateur'] == 'oui'))
 {
   ...
   $supprimepar = format($_POST['supprimepar']);
 ...
 if (affichable ($rec))
 {
  echo "<h2>Discussion " . $rec->titre . "</h2>";
  echo '<div style="background:'.$couleur_fond_entete.';">';
  echo "Auteur : " . $rec->auteur . "<br>Date : " . format_date($rec->date);
  if ($_SESSION['moderateur'] == 'oui')
  {
  ...
  if (isset($_POST['auteur']))
  {
   ...
   else
   {
    ...
    $couleur = $couleur_fond_message;
   	$query = "INSERT INTO forums (forum, titre, discussion, reponse, auteur, date, statut, texte) VALUES ('$forum', '" . format_query($rec->titre) . "', " .  $rec->numero . ", 1, '" . format($_POST['auteur']) . "', date('now'), 0, '" . format($_POST['texte']) . "')";
    if ($trace) { echo "<br>$query<br>";  }
    query ($query);
   }
  }
...
Votre nom : <input type=text name=auteur size=60 value="<?php echo $auteur; ?>"><br>
 ...
 if (affichable ($rec))
 {
  ...
  while ($rec = fetch_object($data))
  {
   if (affichable ($rec))
   {
    echo "<br>";
    echo '<div style="background:'.$couleur_fond_entete.';">';
    echo "Auteur : " . $rec->auteur . "<br>Date : " . $rec->date;
    if ($_SESSION['moderateur'] == 'oui')
    {
     echo "<br>Statut : " . libelle_statut($rec->statut);
     if ($rec->statut == 1)
